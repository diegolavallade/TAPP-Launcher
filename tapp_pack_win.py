...
def run(cmd: list[str], cwd: Path) -> None:
    """
    Ejecuta un comando de forma robusta en Windows/macOS/Linux.

    Nota (Windows):
    - npm/pnpm/yarn normalmente son .cmd/.bat. En algunos entornos, CreateProcess no los lanza bien
      si no van envueltos en cmd.exe. Aquí lo resolvemos automáticamente.
    - También validamos que cwd exista (si no, WinError 2).
    """
    if not cwd.exists():
        die(f"El directorio de trabajo (cwd) no existe: {cwd}")
    ...
    if os.name == "nt":
        # Resolver ejecutable real en PATH (incluye .cmd/.bat)
        resolved = shutil.which(exe) or shutil.which(exe + ".cmd") or shutil.which(exe + ".bat")
        if resolved is None:
            die(f"No encuentro '{exe}' en PATH (ni .cmd/.bat).")

        # Si es script .cmd/.bat, ejecutar vía cmd.exe /c para máxima compatibilidad
